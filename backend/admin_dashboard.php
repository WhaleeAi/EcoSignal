...
try {
    ...
    $statsStmt = $pdo->prepare("
        SELECT
            ...
            COUNT(*) FILTER (
                WHERE status IN ('confirmed', 'in_progress', 'resolved', 'rejected')
    ...
    jsonResponse([
        ...
        'stats' => [
            ...
            'assigned' => (int)($statsRow['assigned_count'] ?? 0),
            'reviewed_24h' => (int)($statsRow['reviewed_24h'] ?? 0),
            'reviewed_7d' => (int)($statsRow['reviewed_7d'] ?? 0),
        ],
        'appeals' => $appeals,
    ]);
} catch (Throwable $e) {
